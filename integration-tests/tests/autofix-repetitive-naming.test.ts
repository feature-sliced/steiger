...
`,
  ...
  // Widgets whose main file is named after the slice, loaded with `require` at the top level and
  // inside a function.
  'src/widgets/HeaderBlock/ui/HeaderBlock.tsx': `export const HeaderBlock = () => null\n`,
  'src/widgets/HeaderBlock/index.ts': `export { HeaderBlock } from './ui/HeaderBlock'\n`,
  'src/widgets/FooterBlock/ui/FooterBlock.tsx': `export const FooterBlock = () => null\n`,
  'src/widgets/FooterBlock/index.ts': `export { FooterBlock } from './ui/FooterBlock'\n`,
  'src/widgets/SidebarBlock/ui/SidebarBlock.tsx': `export const SidebarBlock = () => null\n`,
  'src/widgets/SidebarBlock/index.ts': `export { SidebarBlock } from './ui/SidebarBlock'\n`,
  'src/app/ui/legacy.js': `const { HeaderBlock } = require('@/widgets/HeaderBlock')

function loadFooter() {
  return require('@/widgets/FooterBlock')
}

module.exports = { HeaderBlock, loadFooter }
`,
}
...
test('leaves alone the paths inside a slice that name a file after the slice', async () => {
  expect(await fs.readdir(join(project, 'src/widgets'))).toEqual(['Footer', 'Header', 'Sidebar'])
  expect(await fs.readdir(join(project, 'src/widgets/Header/ui'))).toEqual(['HeaderBlock.tsx'])
  expect(await fs.readFile(join(project, 'src/widgets/Header/index.ts'), 'utf8')).toBe(
    `export { HeaderBlock } from './ui/HeaderBlock'\n`,
  )
})

test('rewrites `require` calls at the top level and inside functions', async () => {
  expect(await fs.readFile(join(project, 'src/app/ui/legacy.js'), 'utf8'))
    .toBe(`const { HeaderBlock } = require('@/widgets/Header')

function loadFooter() {
  return require('@/widgets/Footer')
}
