...
`,
  ...
  // Slices whose main file is named after the slice itself.
  '/pages/pascal/HomePage/index.ts': "export { HomePage } from './ui/HomePage'",
  '/pages/pascal/HomePage/ui/HomePage.tsx': 'export const HomePage = () => null',
  '/pages/pascal/AboutPage/index.ts': "export { AboutPage } from './ui/AboutPage'",
  '/pages/pascal/AboutPage/ui/AboutPage.tsx': 'export const AboutPage = () => null',
  '/pages/pascal/ContactPage/index.ts': "export { ContactPage } from './ui/ContactPage'",
  '/pages/pascal/ContactPage/ui/ContactPage.tsx': 'export const ContactPage = () => null',
  '/app/pascal.ts': "import { AboutPage } from '@/pages/pascal/AboutPage/ui/AboutPage'\n",

  // Slices loaded with `require`, at the top level and nested in functions and blocks.
  '/features/legacy/loginModal/index.ts': '',
  '/features/legacy/signupModal/index.ts': '',
  '/features/legacy/resetModal/index.ts': '',
  '/app/legacy.js': `const login = require('@/features/legacy/loginModal')
require('@/features/legacy/signupModal')
function openReset() {
  return require('@/features/legacy/resetModal')
}
if (process.env.DEBUG) {
  const debugLogin = require('@/features/legacy/loginModal')
}
`,

  // The rename target exists on disk, but the rule doesn't see it (say, because the config ignores it).
  '/entities/hidden/userBadge/index.ts': '',
  '/entities/hidden/postBadge/index.ts': '',
  '/entities/hidden/commentBadge/index.ts': '',
  '/entities/hidden/user/index.ts': '',
}
...
`)
  ...
  it('leaves alone a path inside a slice that names a file after the slice', async () => {
    const root = parseIntoFsdRoot(`
      📂 pages
        📂 pascal
          📂 HomePage
    ...
    const diagnostic = await diagnosticFor(root, joinFromRoot('pages', 'pascal'))

    // Only the folder is renamed, so `./ui/HomePage` still points at `Home/ui/HomePage.tsx`.
    const after = applyFixes(diagnostic.fixes!)
    expect(after[joinFromRoot('pages', 'pascal', 'Home', 'ui', 'HomePage.tsx')]).toBeDefined()
    expect(after[joinFromRoot('pages', 'pascal', 'Home', 'index.ts')]).toBe("export { HomePage } from './ui/HomePage'")
    expect(after[joinFromRoot('pages', 'pascal', 'About', 'index.ts')]).toBe(
      "export { AboutPage } from './ui/AboutPage'",
    )
    expect(after[joinFromRoot('pages', 'pascal', 'Contact', 'index.ts')]).toBe(
      "export { ContactPage } from './ui/ContactPage'",
    )
  })

  it('rewrites only the segment that names the slice folder when a file is named after the slice too', async () => {
    const root = parseIntoFsdRoot(`
      📂 pages
        📂 pascal
          📂 HomePage
    ...
    const after = applyFixes(diagnostic.fixes!)
    expect(after[joinFromRoot('app', 'pascal.ts')]).toBe(
      "import { AboutPage } from '@/pages/pascal/About/ui/AboutPage'\n",
    )
  })

  it('rewrites `require` calls at the top level and nested in functions and blocks', async () => {
    const root = parseIntoFsdRoot(`
      📂 features
        📂 legacy
          📂 loginModal
    ...
    const diagnostic = await diagnosticFor(root, joinFromRoot('features', 'legacy'))

    const after = applyFixes(diagnostic.fixes!)
    expect(after[joinFromRoot('app', 'legacy.js')]).toBe(`const login = require('@/features/legacy/login')
require('@/features/legacy/signup')
function openReset() {
  return require('@/features/legacy/reset')
}
if (process.env.DEBUG) {
  const debugLogin = require('@/features/legacy/login')
}
`)
  })
})
...
describe('leaves the ambiguous cases alone', () => {
  ...
  it('does not offer a fix when the new name is taken on disk by a folder the rule does not see', async () => {
    // `user` exists on disk but is missing from the tree the rule gets, as with a folder the config ignores.
    const root = parseIntoFsdRoot(`
      📂 entities
        📂 hidden
          📂 userBadge
    ...
    const diagnostic = await diagnosticFor(root, joinFromRoot('entities', 'hidden'))
    expect(diagnostic.fixes).toBeUndefined()
  })
