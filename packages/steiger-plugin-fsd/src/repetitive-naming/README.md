...
- the new name is already taken by something else in the same layer or group, including when the two names
  differ only in case, and including folders that the configuration excludes from linting;
...
- a reference to the slice can't be rewritten with certainty, because the specifier doesn't resolve or
  because no single change of a slice name in it resolves to the same file after the rename.

A reference that still resolves to the same file after the rename is left as it is. For example,
`./ui/HomePage` inside the `HomePage` slice keeps pointing at `Home/ui/HomePage.tsx`.

Steiger only updates references inside the folder it was pointed at, and skips the files that the
configuration excludes from linting. If one of those, a build config or an ignored test for instance,
refers to a slice path, you have to update that by hand.
