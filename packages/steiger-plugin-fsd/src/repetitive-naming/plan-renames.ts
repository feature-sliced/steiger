...
export function planRenames(candidate: RepetitiveWordCandidate, root: Folder): Array<SliceRename> | null {
  ...
  // The tree the rule gets can leave out folders that are still on disk, such as the ones the config
  // ignores, and a rename onto one of those fails just the same.
  if (renames.some((rename) => fs.existsSync(join(candidate.groupPath, rename.newName)))) {
    return null
  }
 ...
 *
 * With two repetitive words in one group, both describe the same slice folders, and there's no
 * telling which one the author meant to get rid of. Removing one would also leave the other one
 * reported. Such groups stay as plain diagnostics.
 */
