...
it('should take a threshold from rule options and report errors on a project with shared/lib above the threshold', () => {
  const root = parseIntoFsdRoot(`
    📂 entities
      📂 users
        📂 ui
  ...
  const { diagnostics } = excessiveSlicing.check(root, undefined, { threshold: 10 })

  expect(diagnostics).toEqual([
    {
      message: 'Shared/lib has 11 modules, which is above the recommended threshold of 10. Consider grouping them.',
      location: { path: joinFromRoot('shared', 'lib') },
    },
  ])
})
